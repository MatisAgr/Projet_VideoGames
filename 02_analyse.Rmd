...
df_reco <- df %>% filter(!is.na(Recommendations))

mean(df_reco$Recommendations, na.rm = TRUE)
median(df_reco$Recommendations, na.rm = TRUE)
sum(df_reco$Recommendations > 0, na.rm = TRUE)

df_reco %>% arrange(desc(Recommendations)) %>% slice(1:10) %>% select(Name, Recommendations, Price, User.score)
```
...
# 14. Distribution des évaluations Metacritic vs User Score

```{r score_comparison}
df_score <- df %>% filter(!is.na(Metacritic.score) & !is.na(User.score))

score_bins <- data.frame(
  Metacritic = cut(df_score$Metacritic.score, breaks = seq(0, 100, 10)),
  User = cut(df_score$User.score, breaks = seq(0, 100, 10))
)
score_bins <- score_bins %>% filter(!is.na(Metacritic) & !is.na(User))

score_comparison <- score_bins %>%
  group_by(Metacritic) %>%
  summarise(Count = n(), .groups = 'drop')

if(nrow(score_comparison) > 0) {
  ggplot(score_comparison, aes(x = Metacritic, y = Count, fill = Metacritic)) +
    geom_bar(stat = "identity") +
    labs(title = "Distribution des scores Metacritic par groupes", x = "Gamme de scores", y = "Nombre de jeux") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    guides(fill = "none")
}
...
# 15. Analyse des plates-formes par année
...
# 16. Analyse de la qualité vs popularité

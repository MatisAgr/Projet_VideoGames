...
```{r scores_analysis}
df_scores <- df %>% filter((!is.na(Metacritic.score) & Metacritic.score > 0) | (!is.na(User.score) & User.score > 0))

df_scores$Metacritic.score <- as.numeric(df_scores$Metacritic.score)
df_scores$User.score <- as.numeric(df_scores$User.score)

df_scores_clean <- df_scores %>% filter(Metacritic.score > 0 | User.score > 0)

mean(df_scores_clean$Metacritic.score, na.rm = TRUE)
median(df_scores_clean$Metacritic.score, na.rm = TRUE)
mean(df_scores_clean$User.score, na.rm = TRUE)
median(df_scores_clean$User.score, na.rm = TRUE)

valid_scores <- df_scores_clean %>% filter(!is.na(Metacritic.score) & !is.na(User.score) & Metacritic.score > 0 & User.score > 0)
if(nrow(valid_scores) > 0) {
  cor(valid_scores$Metacritic.score, valid_scores$User.score)
}

df_meta <- df_scores_clean %>% filter(!is.na(Metacritic.score) & Metacritic.score > 0)
if(nrow(df_meta) > 0) {
...
df_user <- df_scores_clean %>% filter(!is.na(User.score) & User.score > 0)
if(nrow(df_user) > 0) {
...
df_pq <- df %>% filter(!is.na(Price) & Price > 0)

df_pq$Price_Category <- cut(df_pq$Price, breaks = c(0, 10, 20, 50, Inf), labels = c("Budget", "Standard", "Premium", "Luxe"))

price_quality <- df_pq %>% 
  filter(!is.na(Price_Category) & !is.na(Metacritic.score) & Metacritic.score > 0) %>%
...
# 15. Analyse de la qualité

```{r quality_analysis}
df_quality <- df %>% filter(!is.na(Metacritic.score) & Metacritic.score > 0 & !is.na(Average.playtime.forever) & Average.playtime.forever > 0)

if(nrow(df_quality) > 0) {
  ggplot(df_quality, aes(x = Metacritic.score, y = Average.playtime.forever, color = Price)) +
...
# 16. Analyse de la popularité

```{r popularity_analysis}
df_popularity <- df %>% filter(!is.na(User.score) & User.score > 0 & !is.na(Average.playtime.forever) & Average.playtime.forever > 0)

if(nrow(df_popularity) > 0) {
  ggplot(df_popularity, aes(x = User.score, y = Average.playtime.forever, color = Price)) +
    geom_point(alpha = 0.4, size = 2) +
    geom_smooth(method = "lm", se = FALSE, color = "blue", linewidth = 1) +
    ylim(0, 1000) +
    labs(title = "Score Utilisateur vs Temps de jeu moyen", x = "Score Utilisateur", y = "Temps de jeu moyen (heures)", color = "Prix (€)") +
    theme_minimal() +
    scale_color_gradient(low = "lightgreen", high = "darkred")
}
```

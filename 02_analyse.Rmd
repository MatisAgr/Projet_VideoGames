...
```{r quality_analysis}
df_quality <- df %>% filter(!is.na(Metacritic.score) & Metacritic.score > 0)
...
df_qvp_user <- df %>% filter(!is.na(User.score) & User.score > 0 & !is.na(Average.playtime.forever) & Average.playtime.forever > 0)

if(nrow(df_qvp_user) > 0) {
  ggplot(df_qvp_user, aes(x = User.score, y = Average.playtime.forever, color = Price)) +
    geom_point(alpha = 0.4, size = 2) +
